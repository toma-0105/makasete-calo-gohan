...
# scaling_type を明示的に渡すと名前による自動分類より優先する
# （例: グラム表記があっても完成された一皿として増量させたくない一品料理）
def create_meal_masters(items, meal_timing:, category:, scaling_type: nil)
  items.each do |name, calories|
    MealMaster.create!(
      name: name, calories: calories, meal_timing: meal_timing, category: category,
      scaling_type: scaling_type || scaling_type_for(name), genre: genre_for(name)
    )
  end
end
...
# 以下は完成された一皿として増量させない（グラム表記はあっても scaling_type は fixed #135）
create_meal_masters(
  {
    "ナポリタン(350g)" => 550,
    "ペペロンチーノ(300g)" => 480,
    ...
    "ハヤシライス(400g)" => 600,
    "麻婆豆腐丼(400g)" => 580,
    "ツナとキャベツのトマトソースオートミール(250g)" => 270,
    "キャベツのお好み焼き(250g)" => 400,
    "ビビンバ(400g)" => 580,
    "しらすと青じそのスパゲッティ(300g)" => 480,
    "いわし缶の和風スパゲッティ(350g)" => 550,
    "たらことえのきのパスタ(350g)" => 520,
    "オートミールオムライス(300g)" => 320,
    "オートミール中華粥(350g)" => 220,
    "オートミールと鶏胸肉の中華粥(350g)" => 250
  }, meal_timing: :lunch_or_dinner, category: :one_dish, scaling_type: :fixed
)
...
link_meal_allergens(
  {
    ...
    "ハヤシライス(400g)" => [ "牛肉" ],
    "麻婆豆腐丼(400g)" => [ "大豆" ],
    "キャベツのお好み焼き(250g)" => %w[小麦 卵],
    "ビビンバ(400g)" => [ "卵" ],
    "しらすと青じそのスパゲッティ(300g)" => [ "小麦" ],
    "いわし缶の和風スパゲッティ(350g)" => [ "小麦" ],
    "たらことえのきのパスタ(350g)" => [ "小麦" ],
    "オートミールオムライス(300g)" => [ "卵" ],
    "オートミールの枝豆とチーズの塩昆布おにぎり1個" => %w[乳 大豆],
    "オートミールと鶏胸肉の中華粥(350g)" => [ "鶏肉" ]
  }
)
